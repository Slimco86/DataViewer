...
def update(*args):
    ...
    for id in data.keys():
        if args[0]:
            box = data[id]['person_bbox']
            cv2.rectangle(img,(box[0],box[1]),(box[2],box[3]),(0,0,255),2)
        if args[1]:
            box = data[id]['face_bbox']
            cv2.rectangle(img,(box[0],box[1]),(box[2],box[3]),(0,255,0),2)
            if args[4]:
                cv2.putText(img,str(data[id]['race']),(box[0],box[1]+10),cv2.FONT_HERSHEY_SIMPLEX,0.5,(255,255,255),1,cv2.LINE_AA)
            if args[-1]:
                cv2.putText(img,str(data[id]['age']),(box[0],box[1]+20),cv2.FONT_HERSHEY_SIMPLEX,0.5,(255,255,255),1,cv2.LINE_AA)

        if args[2]:
            landmarks = np.array(data[id]['pose'])
            for p in range(landmarks.shape[0]):
                cv2.circle(img,(landmarks[p][0],landmarks[p][1]),3,(150,255,0),cv2.FILLED)
                

        if args[3]:
            landmarks = np.array(data[id]['face_landmarks'])      
            for p in range(landmarks.shape[0]):
                cv2.circle(img,(landmarks[p][0],landmarks[p][1]),3,(150,0,255),cv2.FILLED)
